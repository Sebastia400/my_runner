...
int hit_the_character(t_var *var)
{
    ...
    sfVector2f posActualgohst = var->pos_gohst;

    if (posActualcharacter.y <= posActualgohst.y + 50
    && posActualcharacter.y >= posActualgohst.y - 50
    && posActualcharacter.x <= posActualgohst.x + 50
    && posActualcharacter.x >= posActualgohst.x - 50) {;
        return (1);
    }
    return (0);
